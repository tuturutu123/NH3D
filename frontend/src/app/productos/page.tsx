...
export default async function ProductosPage({ searchParams }: { searchParams?: any }) {
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] dark:bg-[#0f172a] py-12">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            <p className="text-sm uppercase tracking-[0.25em] text-[#627653] dark:text-[#6ba368] font-semibold">Catálogo</p>
            <h1 className="text-4xl font-extrabold text-[#2a3c2e] dark:text-[#a8d5a2]">Productos</h1>
          </div>
          <div className="flex items-center gap-3 rounded-full border border-[#e2dfd3] dark:border-gray-700 bg-white dark:bg-[#1e293b] px-4 py-3 shadow-sm w-full md:w-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {filtrados.map((producto: any) => (
            <div key={producto.id} className="bg-white dark:bg-[#1e293b] rounded-2xl p-4 shadow-sm border border-[#ece7dc] dark:border-gray-700 hover:shadow-lg transition-all duration-200">
              ...
              <p className="text-[10px] uppercase tracking-[0.18em] text-[#6c7b69] dark:text-[#6ba368] font-semibold mb-2">{producto.categoria?.nombre || 'General'}</p>
              <h3 className="text-lg font-bold text-[#2a3c2e] dark:text-gray-100 mb-2 min-h-[48px]">{producto.nombre}</h3>
              ...
              <div className="flex gap-2">
                <Link href={`/productos/${producto.id}`} className="flex-1 bg-[#324b3b] dark:bg-[#6ba368] text-white py-2.5 rounded-full text-center font-semibold hover:bg-[#24382d] dark:hover:bg-[#5a9455] transition-colors">
                  Ver
                </Link>
                <Link href={`/carrito`} className="w-12 border border-[#dfe5de] dark:border-gray-600 rounded-full flex items-center justify-center text-[#324b3b] dark:text-[#6ba368] hover:bg-[#f1f4ee] dark:hover:bg-gray-700 transition-colors" aria-label="Agregar al carrito">
        ...
        {filtrados.length === 0 && (
          <div className="mt-10 text-center bg-white dark:bg-[#1e293b] rounded-2xl border border-[#ece7dc] dark:border-gray-700 py-10">
