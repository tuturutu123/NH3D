  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center container mx-auto px-4 text-center">
        <div className="bg-[#f5f4ef] p-6 rounded-full text-[#324b3b] mb-4">
          <ShoppingBag className="h-12 w-12" />
        </div>
        <h2 className="text-2xl font-bold text-[#2a3c2e] mb-2">Tu carrito está vacío</h2>
        ...
        <Link 
          href="/" 
          className="bg-[#324b3b] hover:bg-[#233328] text-white font-bold py-3 px-8 rounded-full transition-colors flex items-center gap-2 text-sm shadow-md"
        >
    ...
    <div className="bg-[#faf9f6] min-h-screen py-10">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-extrabold text-[#2a3c2e] tracking-tight">Carrito de Compras</h1>
          <Link href="/" className="text-sm font-semibold text-[#627653] hover:text-[#2a3c2e] flex items-center gap-1 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white border border-[#eae6db] rounded-2xl p-4 flex items-center gap-4 shadow-sm">
                <div className="h-20 w-20 rounded-xl bg-[#fcfbf9] border border-gray-100 overflow-hidden shrink-0 flex items-center justify-center p-2">
                  {item.imagenUrl ? (
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900 text-sm truncate mb-1">{item.nombre}</h3>
                  <p className="text-sm font-bold text-[#2a3c2e]">${item.precio.toLocaleString('es-AR')}</p>
                </div>
                ...
                <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50">
                  <button 
                    ...
                    className="p-1.5 text-gray-500 hover:text-black transition-colors disabled:opacity-30"
                  >
                    <Minus className="h-3.5 w-3.5" />
                  </button>
                  <span className="px-3 text-sm font-bold text-gray-800">{item.cantidad}</span>
                  <button 
                    onClick={() => updateQuantity(item.id, 1)}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-[#eae6db] rounded-2xl p-6 shadow-sm sticky top-28">
              <h2 className="text-lg font-bold text-[#2a3c2e] mb-4 border-b border-gray-100 pb-3">Resumen del pedido</h2>
              ...
              <div className="space-y-3 mb-6 text-sm">
                ...
                <div className="border-t border-gray-100 pt-3 flex justify-between text-base font-extrabold text-gray-900">
                  <span>Total</span>
                  <span className="text-[#2a3c2e]">${totalGeneral.toLocaleString('es-AR')}</span>
                </div>
              </div>
